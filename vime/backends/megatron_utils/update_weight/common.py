...
def _named_params_and_buffers_vanilla(model: Sequence[torch.nn.Module]) -> Iterator[tuple[str, torch.Tensor]]:
    for vp_stage, model_module in enumerate(model):
        ...
        for name, param in model_module.named_parameters():
            # Skip DSpark draft model parameters — they use a separate weight sync path
            if name.startswith("draft_model.") or ".draft_model." in name:
                continue
...
def _named_params_and_buffers_global(
    ...
    Skips DSpark draft_model parameters (they use a separate weight sync path).
    """
